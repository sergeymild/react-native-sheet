...
import { FittedSheet } from "./index.js";
import { jsx as _jsx, Fragment as _Fragment } from "react/jsx-runtime";
export function presentGlobalFittedSheet(params) {
  console.log('[presentGlobalFittedSheet] called', {
    name: params.name
  });
  presentGlobalSheet?.(params);
}
export function dismissGlobalFittedSheet(name) {
  console.log('[dismissGlobalFittedSheet] called', {
    name
  });
  dismissGlobalSheet?.(name);
}
export function attachScrollViewToGlobalFittedSheet(name) {
  console.log('[attachScrollViewToGlobalFittedSheet] called', {
    name
  });
  const ref = sheetRefs.get(name);
  if (!ref) {
    console.warn(`[attachScrollViewToGlobalFittedSheet] Sheet "${name}" not found`);
    return false;
  }
...
let forceUpdate;
let presentGlobalSheet;
let dismissGlobalSheet;

// Global stable callbacks
const handleSheetRef = (name, ref) => {
  if (ref) {
...
const GlobalSheetItem = /*#__PURE__*/memo(({
  name,
  defaultProps
}) => {
  console.log('[GlobalSheetItem] render', {
    name
  });
  const dataToRender = sheets.get(name);
  if (!dataToRender) {
    console.log('[GlobalSheetItem] no data, returning null', {
      name
    });
    return null;
  }
  return /*#__PURE__*/_jsx(FittedSheet, {
    ref: ref => handleSheetRef(name, ref),
    name: name,
    params: dataToRender.sheetProps?.params ?? defaultProps?.params,
    rootViewStyle: dataToRender.sheetProps?.rootViewStyle ?? defaultProps?.rootViewStyle,
    onSheetDismiss: () => handleSheetDismiss(name),
    children: dataToRender?.children
  });
}, (prevProps, nextProps) => {
  // Custom comparison to log when memo blocks re-renders
  const shouldSkip = prevProps.name === nextProps.name && prevProps.defaultProps === nextProps.defaultProps;
...
export const GlobalSheetView = /*#__PURE__*/memo(props => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      presentGlobalSheet = () => {
        console.warn('[GlobalSheetView] presentGlobalSheet called after unmount!');
      };
      dismissGlobalSheet = () => {
        console.warn('[GlobalSheetView] dismissGlobalSheet called after unmount!');
      };
      forceUpdate = () => {};
  ...
  return /*#__PURE__*/_jsx(_Fragment, {
    children: Array.from(sheets.keys()).map(name => /*#__PURE__*/_jsx(GlobalSheetItem, {
      name: name,
      defaultProps: props.props
    }, name))
  });
});
//# sourceMappingURL=GlobalSheetView.js.map