...
class SheetModule(reactContext: ReactApplicationContext) : NativeSheetSpec(reactContext) {
  ...
  override fun dismissAll() {
    reactApplicationContext.runOnUiQueueThread {
      reactApplicationContext.currentActivity?.let { AppFittedSheet.Companion.dismissAll(it as AppCompatActivity) }
    }
  }

  override fun dismissPresented() {
    reactApplicationContext.runOnUiQueueThread {
      reactApplicationContext.currentActivity?.let { AppFittedSheet.Companion.dismissPresented(it as AppCompatActivity) }
    }
  }
}
