 ...
 *
 * `contentOriginOffset` is the visual delta between the Yoga-computed
 * position of the sheet content and its actual on-screen position after the
 * iOS host view has been re-parented via child-VC containment for inline
 * presentation. It is pushed from the Swift side (`HostFittedSheet`) on
 * containment-settle, and consumed by
 * `SheetViewShadowNode::getContentOriginOffset()` so that descendants'
 * `measure()` and Pressability region checks report the real screen
 * position.
 */
