...
public class SheetViewController: UIViewController {
    ...
    public var currentSize: SheetSize? {
        didSet {
            debugPrint("currentSize", self.height(for: currentSize))
        }
    }
    ...
    public var minimumSpaceAbovePullBar: CGFloat {
        didSet {
            if self.isViewLoaded, let currentSize {
                self.resize(to: currentSize)
            }
        }
    }
    ...
    public init(
        ...
        self.sizes = sizes
        self.currentSize = sizes[currentSize ?? sizes.count - 1]
    ...
    public override func viewDidLoad() {
        ...
        self.resize(to: currentSize ?? self.sizes.first ?? .intrinsic, animated: false)
    }
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        if let currentSize { self.resize(to: currentSize, animated: false) }
    }
    ...
    public func setSizes(
        ...
        animated: Bool = true,
        currentSize: Int? = nil,
        skipHeightConstraint: Bool = false
    ) {
        ...
        self.resize(
            to: self.currentSize!,
            animated: animated,
            skipHeightConstraint: skipHeightConstraint
        )
    }
        ...
        switch gesture.state {
            ...
            case .ended:
                ...
                UIView.animate(
                    ...
                    self.isPanning = false
                    if let newSize, previousSize != newSize {
    ...
    private func adjustForKeyboard(height: CGFloat, from notification: Notification) {
        ...
        guard let currentSize else { return }
        self.resize(to: currentSize, duration: duration, options: animationCurve, animated: true, complete: {
            self.resize(to: currentSize)
        })
    }
    
    private func height(for size: SheetSize?, skipHeightConstraint: Bool = false) -> CGFloat {
        ...
        if skipHeightConstraint { return contentHeight }
    ...
    public func resize(to size: SheetSize,
        ...
        let newHeight = self.height(for: size, skipHeightConstraint: skipHeightConstraint)
    ...
    public func animateIn(to view: UIView, size: SheetSize? = nil, duration: TimeInterval = 0.3, completion: (() -> Void)? = nil) {
...
//        self.didMove(toParent: parent)
    ...
    public func animateIn(size: SheetSize? = nil, duration: TimeInterval = 0.3, completion: (() -> Void)? = nil) {
        ...
        self.contentViewController.updatePreferredHeight()
        if let s = size ?? self.sizes.first ?? self.currentSize {
            self.resize(to: s, animated: false)
        }
...
extension SheetViewController: SheetContentViewDelegate {
    func pullBarTapped() {
        ...
        if self.sizes.count > 1, let currentSize {
            let index = (self.sizes.firstIndex(of: currentSize) ?? 0) + 1
